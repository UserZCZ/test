package com.atguigu.java;

/**
 * @author shkstart
 * @create 2019-09-25 9:42
 *
 * 1.IDEA中代码模板所处的位置：settting->Editor->Live Templates/Postfix Completion
 * 2.常用的模板
 */
public class TemplatesTest {
/*

    //模板六：prsf
    private static final Customer cust = new Customer();
    //变形：psf
    public static final int NUM = 1;
    //变形：psfi
    public static final int NUM2 = 2;
    //变形： psfs
    public static final String NATION = "China";


    //模板一：psvm
    public static void main(String[] args) {

        //模板二：sout
        System.out.println("hello!");
        //变形：soutp / soutm / soutv / xxx.sout
        System.out.println("args = " + Arrays.deepToString(args));      //soutp
        System.out.println("TemplatesTest.main");                       //soutm

        int num1 = 10;
        System.out.println("num1 = " + num1);                           //soutv
        System.out.println(num1);                                       //num1.sout

        //模板三：fori
        String[] arr = new String[]{"Tom", "Jerry", "LiLei"};
        for (int i = 0; i < arr.length; i++) {
            System.out.println(arr[i]);
        }
        //变形：iter
        for (String s : arr) {
            System.out.println(s);
        }
        //变形：itar
        for (int i = 0; i < arr.length; i++) {
            String s = arr[i];
            System.out.println(s);
        }

        //模板四：list.for
        ArrayList list = new ArrayList();
        list.add(123);
        list.add(345);
        list.add(567);
        for (Object o : list) {                         //增强for循环

        }
        //变形：list.fori
        for (int i = 0; i < list.size(); i++) {         //普通for循环

        }
        //变形：list.forr
        for (int i = list.size() - 1; i >= 0; i--) {    //逆序for循环

        }


    }

    public void method() {
        System.out.println("TemplatesTest.method");                     //soutm

        //模板五：ifn
        if (list == null) {

        }
        //变形：inn
        if (list != null) {

        }
        //变形：xxx.nn / xxx.null


    }

*/


}
